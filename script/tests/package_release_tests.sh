...
if [ ! -x "$RELEASE_SCRIPT" ]; then
    ...
    assert_contains "$RELEASE_SCRIPT" 'Qrecs-0.1.0-macOS.zip' "release archive name is not pinned to v0.1.0"
fi
...
EOF
    ...
    if ! validate_release_metadata "$APP_BUNDLE" "0.1.0" "15.0" >/dev/null; then
        fail "valid v0.1.0/macOS 15 metadata was rejected"
    fi
    if validate_release_metadata "$APP_BUNDLE" "0.2.0" "15.0" >/dev/null 2>&1; then
        fail "incorrect bundle version was accepted"
    fi
    if validate_release_metadata "$APP_BUNDLE" "0.1.0" "14.0" >/dev/null 2>&1; then
...
<?xml version="1.0"?><plist><dict><key>com.apple.security.app-sandbox</key><true/></dict></plist>'
    ...
    printf 'qrecs release fixture\n' > "$TEST_DIR/Qrecs-0.1.0-macOS.zip"
    write_sha256 "$TEST_DIR/Qrecs-0.1.0-macOS.zip" "$TEST_DIR/Qrecs-0.1.0-macOS.zip.sha256"
    expected_checksum="$(shasum -a 256 "$TEST_DIR/Qrecs-0.1.0-macOS.zip" | awk '{print $1}')  Qrecs-0.1.0-macOS.zip"
    actual_checksum="$(tr -d '\n' < "$TEST_DIR/Qrecs-0.1.0-macOS.zip.sha256")"
